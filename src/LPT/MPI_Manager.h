...
public:
    ...
    void Init(const MPI_Comm& comm_particle = MPI_COMM_WORLD, const MPI_Comm& comm_fluid = MPI_COMM_WORLD)
    {
        ...
        //myrank_p -> myrank_wの変換テーブルが問題無いか確認
        for(int i=0; i<nproc_in_particle;i++)
        {
        ...
        //myrank_f -> myrank_wの変換テーブルが問題無いか確認
        for(int i=0; i<nproc_in_fluid;i++)
        {
...
public:
    ...
    bool is_particle_proc(void){return myrank_in_particle != MPI_UNDEFINED;}

    //! 流体計算を担当するプロセスかどうかを返す
    bool is_fluid_proc(void){return myrank_in_fluid != MPI_UNDEFINED;}
