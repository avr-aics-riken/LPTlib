...
void Rectangle::Divider(std::vector<StartPoint*>* StartPoints, const int& MaxNumStartPoints)
{
    ...
    {
        Rectangle* NewRectangel = RectangleFactory(this->Coord1, this->Coord2, this->NumStartPoints, StartTime, ReleaseTime, TimeSpan, ParticleLifeTime);
    ...
    if(N%NB != 0)
    {
        ...
        ReminderN->SumStartPoints = ((ReminderN->NumStartPoints)[0]*(ReminderN->NumStartPoints)[1]*(ReminderN->NumStartPoints)[2]);
    ...
    if(M%MB != 0)
    {
        ...
        ReminderM->SumStartPoints = ((ReminderM->NumStartPoints)[0]*(ReminderM->NumStartPoints)[1]*(ReminderM->NumStartPoints)[2]);
    ...
    for(std::vector<Rectangle*>::iterator it = tmpStartPoint.begin(); it != tmpStartPoint.end(); ++it)
    {
        ...
        for(int i = 0; i < orgM/MB; i++)
        {
            ...
            tmpRectangle->SumStartPoints = ((tmpRectangle->NumStartPoints)[0]*(tmpRectangle->NumStartPoints)[1]*(tmpRectangle->NumStartPoints)[2]);
